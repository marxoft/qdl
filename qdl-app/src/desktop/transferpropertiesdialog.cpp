    ...
    m_removeButton(new QPushButton(QIcon::fromTheme("edit-delete"), tr("Remove"), this))
{
    ...
    serviceIcon->setFixedSize(20, 20);
    ...
    grid->addWidget(nameWidget, 0, 0, 1, 3);
    grid->addWidget(new QLabel(QString("<a href='%1'>%1</a>").arg(m_transfer->url().toString())), 1, 0, 1, 3);
    grid->addWidget(m_convertToAudioCheckBox, 2, 0, 1, 3);
    grid->addWidget(new QLabel(tr("Connections") + ":", this), 3, 0);
    grid->addWidget(m_connectionsSelector, 3, 1, 1, 2);
    grid->addWidget(new QLabel(tr("Category") + ":", this), 4, 0);
    grid->addWidget(m_categorySelector, 4, 1, 1, 2);
    grid->addWidget(new QLabel(tr("Priority") + ":", this), 5, 0);
    grid->addWidget(m_prioritySelector, 5, 1, 1, 2);
    grid->addWidget(m_progressLabel, 6, 0, 1, 3);
    grid->addWidget(m_progressBar, 7, 0, 1, 3);
    grid->addWidget(m_statusLabel, 8, 0, 1, 3);
    grid->addWidget(m_startButton, 9, 0);
    grid->addWidget(m_pauseButton, 9, 1);
    grid->addWidget(m_removeButton, 9, 2);
    grid->addWidget(buttonBox, 10, 0, 1, 3);
...
void TransferPropertiesDialog::onTransferDataChanged(int role) {
    ...
    case Transfer::SizeRole:
        ...
        m_progressLabel->setText(m_transfer->size() > 0 ? QString("%1 of %2 (%3%)").arg(Utils::fileSizeFromBytes(m_transfer->position())).arg(Utils::fileSizeFromBytes(m_transfer->size())).arg(m_transfer->progress())
                                                        : QString());
        return;
