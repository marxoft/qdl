    ...
    m_removeButton(new QPushButton(tr("Remove"), this))
{
    ...
    m_view->setFixedHeight(!m_model->rowCount() ? 56 : m_view->sizeHintForRow(0) * m_model->rowCount());
    ...
    grid->addWidget(new QLabel(QString("<b>%1</b>").arg(m_package->packageName()), this), 0, 0, 1, 2);
    grid->addWidget(new QLabel(tr("Category") + ":", this), 2, 0);
    grid->addWidget(m_categorySelector, 2, 1);
    grid->addWidget(new QLabel(tr("Priority") + ":", this), 3, 0);
    grid->addWidget(m_prioritySelector, 3, 1);
    grid->addWidget(new QLabel(tr("Files") + ":", this), 4, 0, 1, 2);
    grid->addWidget(m_view, 5, 0, 1, 2);
