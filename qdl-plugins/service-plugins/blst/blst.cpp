...
QRegExp Blst::urlPattern() const {
    return QRegExp("http(s|)://(www.|)bl.st/\\w+", Qt::CaseInsensitive);
}

bool Blst::urlSupported(const QUrl &url) const {
    return this->urlPattern().indexIn(url.toString()) == 0;
}

void Blst::login(const QString &username, const QString &password) {
    QString data = QString("op=login&redirect=&login=%1&password=%2").arg(username).arg(password);
    QUrl url("https://bl.st");
...
void Blst::checkUrl(const QUrl &url) {
    QNetworkRequest request(url);
...
void Blst::getDownloadRequest(const QUrl &url) {
...
void Blst::getWaitTime() {
    QString data = QString("op=download1&id=%1&fname=%2&method_free=Initiate+Slower+Download").arg(m_fileId).arg(m_fileName);
...
void Blst::checkWaitTime() {
    ...
    if (response.contains("You have to wait")) {
        ...
        if ((secs <= 0) || (m_rand.isEmpty()) || (m_code.isEmpty())) {
            emit error(UnknownError);
        }
        else {
...
void Blst::submitCaptcha() {
    QString data = QString("op=download2&id=%1&fname=%2&rand=%3&method_free=Initiate+Slower+Download&down_script=1&code=%4").arg(m_fileId).arg(m_fileName).arg(m_rand).arg(m_code);
    QNetworkRequest request(m_url);
    request.setHeader(QNetworkRequest::ContentTypeHeader, "application/x-www-form-urlencoded");
    QNetworkReply *reply = this->networkAccessManager()->post(request, data.toUtf8());
...
void Blst::onWaitFinished() {
    emit statusChanged(Ready);
    this->disconnect(this, SIGNAL(waitFinished()), this, SLOT(onWaitFinished()));
}

bool Blst::cancelCurrentOperation() {
    m_waitTimer->stop();
