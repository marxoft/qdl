...
SecureUpload::SecureUpload(QObject *parent) :
...
QRegExp SecureUpload::urlPattern() const {
    return QRegExp("http(s|)://(www.|)secureupload.eu/\\w+", Qt::CaseInsensitive);
}

bool SecureUpload::urlSupported(const QUrl &url) const {
    return this->urlPattern().indexIn(url.toString()) == 0;
}

void SecureUpload::login(const QString &username, const QString &password) {
    QString data = QString("op=login&redirect=&login=%1&password=%2").arg(username).arg(password);
    QUrl url("https://secureupload.eu");
...
void SecureUpload::checkUrl(const QUrl &url) {
    QNetworkRequest request(url);
...
void SecureUpload::checkUrlIsValid() {
    QNetworkReply *reply = qobject_cast<QNetworkReply*>(this->sender());
    ...
    QRegExp re("http(s|)://\\w+.secureupload.eu:\\d+/[^'\"]+");
...
void SecureUpload::getDownloadRequest(const QUrl &url) {
...
void SecureUpload::onWebPageDownloaded() {
    QNetworkReply *reply = qobject_cast<QNetworkReply*>(this->sender());
...
void SecureUpload::checkWaitTime() {
    QNetworkReply *reply = qobject_cast<QNetworkReply*>(this->sender());
    ...
    if (response.contains("You have to wait")) {
        ...
        secs = response.section(QRegExp("waitnigtime=\""), 1, 1).section('"', 0, 0).toInt();
        m_rand = response.section("rand\" value=\"", 1, 1).section('"', 0, 0);

        if ((secs <= 0) || (m_rand.isEmpty())) {
            emit error(UnknownError);
        }
        else {
...
void SecureUpload::downloadCaptcha() {
    emit statusChanged(CaptchaRequired);
    this->disconnect(this, SIGNAL(waitFinished()), this, SLOT(downloadCaptcha()));
}

void SecureUpload::submitCaptchaResponse(const QString &challenge, const QString &response) {
    Q_UNUSED(challenge);

    QString data = QString("op=download2&id=%1&rand=%2&code=%3&method_free=Slow+Download&down_direct=1").arg(m_fileId).arg(m_rand).arg(response);
    QNetworkRequest request(m_url);
    request.setHeader(QNetworkRequest::ContentTypeHeader, "application/x-www-form-urlencoded");
    QNetworkReply *reply = this->networkAccessManager()->post(request, data.toUtf8());
...
void SecureUpload::onCaptchaSubmitted() {
    QNetworkReply *reply = qobject_cast<QNetworkReply*>(this->sender());
...
void SecureUpload::startWait(int msecs) {
    if (msecs > m_waitTime) {
...
void SecureUpload::updateWaitTime() {
...
void SecureUpload::onWaitFinished() {
    emit statusChanged(Ready);
    this->disconnect(this, SIGNAL(waitFinished()), this, SLOT(onWaitFinished()));
}

bool SecureUpload::cancelCurrentOperation() {
    m_waitTimer->stop();
...
Q_EXPORT_PLUGIN2(secureupload, SecureUpload)
#endif
