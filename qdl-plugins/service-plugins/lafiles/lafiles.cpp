...
Lafiles::Lafiles(QObject *parent) :
...
QRegExp Lafiles::urlPattern() const {
    return QRegExp("http(s|)://(www.|)lafiles.com/\\w+", Qt::CaseInsensitive);
}

bool Lafiles::urlSupported(const QUrl &url) const {
    return this->urlPattern().indexIn(url.toString()) == 0;
}

void Lafiles::login(const QString &username, const QString &password) {
    QString data = QString("op=login&redirect=&login=%1&password=%2").arg(username).arg(password);
    QUrl url("http://lafiles.com");
...
void Lafiles::checkUrl(const QUrl &url) {
    QNetworkRequest request(url);
...
void Lafiles::checkUrlIsValid() {
    ...
    QRegExp re("http://\\w+.lafiles.com:\\d+/[^'\"]+");
...
void Lafiles::getDownloadRequest(const QUrl &url) {
...
void Lafiles::onWebPageDownloaded() {
    QNetworkReply *reply = qobject_cast<QNetworkReply*>(this->sender());
...
void Lafiles::downloadCaptcha() {
    emit statusChanged(CaptchaRequired);
    this->disconnect(this, SIGNAL(waitFinished()), this, SLOT(downloadCaptcha()));
}

void Lafiles::submitCaptchaResponse(const QString &challenge, const QString &response) {
...
void Lafiles::onCaptchaSubmitted() {
    QNetworkReply *reply = qobject_cast<QNetworkReply*>(this->sender());
...
void Lafiles::onWaitFinished() {
    emit statusChanged(Ready);
    this->disconnect(this, SIGNAL(waitFinished()), this, SLOT(onWaitFinished()));
}

bool Lafiles::cancelCurrentOperation() {
    m_waitTimer->stop();
...
Q_EXPORT_PLUGIN2(lafiles, Lafiles)
